...
def upload(request, name):
    ...
    if request.method == 'POST':
        # Get the uploaded file, whether it's an image or a video
        media_file = request.FILES.get('media_file')

        # Determine the type of the file (image or video)
        is_video = media_file.content_type.startswith('video/') if media_file else False
        ...
        new_post = Post.objects.create(
            ...
            caption=caption,
            username=username,
            profileimg=profile_obj.profileimg
        )

        # Set the post_image or post_video field based on the file type
        if is_video:
        ...
        new_not = Notification.objects.create(
            new_notification=profile_obj,
            user_from=username,
            user_to=profile_obj.username,
            type=type,
            text=text
        )
        profile_obj.notifications += 1
        new_not.save()
